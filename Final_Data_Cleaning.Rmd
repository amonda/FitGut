...
#No  tree in files online so the commands below will not run at the moment
#plot(phy_tree(ex1), show.node.label = TRUE)
#plot_tree(ex1, color = "SampleType", label.tips = "Rank6", ladderize = "left", justify = "left", size = "Abundance")
...
```{r}
#Alpha diversity of samples by exercise frequency and 
plot_richness(ps, x="ExFreq", color = "", shape = NULL,
               title = NULL, scales = "free_y", nrow = 1, shsi = NULL,
             measures = c("Shannon", "Simpson"), sortby = NULL)
```
...
pssf <- filter_taxa(pss, function(x) sd(x) / mean(x) > 3.0, TRUE)

# Subset to Bacteroidetes

pssfb <- subset_taxa(pssf, Phylum == "Bacteroidetes")
```
